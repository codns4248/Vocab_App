...
public class StudyManager {
    ...
    // 실제 지급은 서버(initializeUser)가 합니다. 보안 규칙이 클라이언트의
    // point 쓰기를 막고 있고, 앱에서 지급하면 얼마든지 다시 부를 수 있습니다.
    public void initNewUserPoint(String userId) {
        com.google.firebase.functions.FirebaseFunctions.getInstance("asia-northeast3")
                ...
                .call()
                .addOnSuccessListener(r -> Log.d("Point", "신규 포인트 지급 요청 완료"))
                .addOnFailureListener(e -> Log.e("Point", "신규 포인트 지급 실패: " + e.getMessage()));
    }
